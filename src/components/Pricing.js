      ...
      <div className="container mx-auto">
        ...
        <div className='flex flex-col mx-auto items-center gap-y-5 lg:flex-row lg:gap-x-[33px]'>
          {pricing.map((card, currentIndex) => {
            // destructure card
            const { title, price, list, buttonIcon, delay, buttonText } = card;
            return (
              ...
              >
                ...
                <div className='px-[30px] pt-[18px] pb-[30px]'>
                    ...
                    {list.map((item, index) => {
                      ...
                      return (
                        <li className='border border-stroke-3 p-4 rounded-md flex items-center gap-4 ' key={index}>
                          ...
                          w-7 h-7 text-2xl text-green-300 rounded-full'>{icon}</div>
                          {name}
                        </li>
                      )
                    })}
                  ...
                  ? 'bg-orange text-white'
                  : ' border-orange' } w-full h-[50px] rounded border border-orange 
                  flex items-center justify-center text-orange font-medium transition relative`}>{buttonText}
                  {/* btn icon */}
                  <div className={`${index === currentIndex
                  ? 'bg-orange-200'
                  : ' bg-orange' } w-[50px] h-[50px] flex justify-center items-center right-0 rounded text-white absolute`}>{buttonIcon}</div>
                  </button>
                  </div>
                </div>
              </div>
